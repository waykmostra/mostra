  ...
  return (
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-sm font-semibold text-ink">Templates de formulaires</h2>
        ...
        <Link
          ...
          className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl bg-brand text-black text-xs font-semibold hover:bg-brand/90 transition-colors flex-shrink-0"
        >
      ...
      {templates.length === 0 && (
        <div className="bg-surface border border-line rounded-2xl p-12 text-center space-y-4">
          ...
          <div>
            <p className="text-sm font-medium text-ink">Aucun template</p>
          ...
          <Link
            href="/settings/forms/new"
            className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-brand text-black text-xs font-semibold hover:bg-brand/90 transition-colors"
          >
            <Plus className="h-3.5 w-3.5" />
            Créer un template
          </Link>
      ...
      {templates.length > 0 && (
        <div className="space-y-2">
          {templates.map((tpl) => (
            <div
              key={tpl.id}
              className="group bg-surface border border-line rounded-xl px-4 py-3.5 flex items-center gap-4 hover:border-line-strong transition-colors"
            >
              {/* Icon */}
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-0.5">
                  {tpl.description && (
                    <span className="text-xs text-faint truncate">{tpl.description}</span>
                  )}
