  ...
  return (
    <div className="bg-surface border border-line rounded-xl overflow-hidden">
      <div className="px-4 sm:px-5 py-3.5 border-b border-line flex items-center justify-between gap-3">
        <div>
          <h2 className="text-sm font-semibold text-ink">Revenus — projets</h2>
        ...
        <div className="flex items-center gap-4 text-right flex-shrink-0">
          <div>
            <p className="text-[10px] uppercase tracking-wider text-faint">Encaissé</p>
            <p className="text-sm font-semibold text-[#22C55E] tabular-nums">{eur(paid)}</p>
          </div>
          <div>
            <p className="text-[10px] uppercase tracking-wider text-faint">En attente</p>
          ...
          {revenues.map((r) => {
            ...
            return (
              <Link
                key={r.id}
                href={`/projects/${r.id}`}
                className="flex items-center gap-3 px-4 sm:px-5 py-3 hover:bg-surface-2 transition-colors group"
              >
                ...
                <span className="text-sm text-ink tabular-nums font-medium w-24 text-right flex-shrink-0">
                  {eur(r.value_eur)}
                </span>
